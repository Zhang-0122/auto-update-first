...
def render_trend(config: LotteryConfig, lottery_id: str, records: list[dict[str, str]]) -> str:
    ...
    if len(recent) < 2:
        ...
        chart = f"""
        <svg class="trend-svg" viewBox="0 0 {width} {height}" role="img" aria-label="近50期一区和值走势">
          {''.join(grid)}
        ...
        """
    return f'<section class="panel" id="trend"><div class="panel-title"><h2>走势分析</h2><span class="muted">只看历史走势，不代表未来结果</span></div>{chart}</section>'
